...
#define BUSY_TIMEOUT_MILLIS 10000
//==============================================================================
...
#define DBG_PRINT()                                                                 \
  {                                                                                 \
    ...
    Serial.println(SDIO->STA, HEX);                                                 \
    Serial.print(", SDIO->FIFOCNT: ");                                              \
    Serial.println(SDIO->FIFOCNT);                                                  \
    /*delay(1);*/                                                                   \
  }
#define DBG_PIN PD0
...
//   sdio_setup_transfer(m_sdClkKhz * 25000, n, flags); // 250ms timeout
// }
/*---------------------------------------------------------------------------*/
static bool trxStop()
{
...
//delay(100);
  ...
  */
  ...
  m_sdClkKhz = 12000; // set clock to 8MHz
...
bool SdioCard::readBlock(uint32_t lba, uint8_t *buf)
{
  ...
  Serial.println(lba); //Serial.print(", buf: "); Serial.println((uint32_t)buf, HEX);
#endif
  uint8_t aligned[512];
  ...
  while (retries--)
  {
    ...
    if (m_curLba != lba)
    {
      ...
      _state = dmaTrxPrepare(isAligned4(buf) ? buf : (uint8_t *)aligned, 512, TRX_RD);
    ...
    if (_state)
    {
      if ((uint32_t)buf & 3)
      {
        ...
        while (i--)
        { // do 8 byte copies, is much faster than single byte copy
          *dst++ = *src++;
          *dst++ = *src++;
          *dst++ = *src++;
          *dst++ = *src++;
...
bool SdioCard::readStop()
{
  ...
  sdio_setup_transfer(m_sdClkKhz * 25000, 0, 0); // 250ms timeout
  // Empty SDIO FIFO
